...
def client(mock_storage):
    ...
    processor = DocumentProcessor(
        provider=TemplateExtractionProvider(), result_repo=_processing_repo
    )
...
class TestExtractedField:
    ...
    def test_confidence_bounds(self):
        with pytest.raises(ValueError):
            ExtractedField(
                field_name="x", value="y", confidence=1.5, needs_review=False
            )
        with pytest.raises(ValueError):
            ExtractedField(
                field_name="x", value="y", confidence=-0.1, needs_review=False
            )
...
class TestTemplateExtractionProvider:
    def test_extracts_skills(self):
        fields = TemplateExtractionProvider().extract(
...
class TestDocumentProcessor:
    def test_process_success(self):
        r = DocumentProcessor().process(
            "doc-1", content="Jane Smith\n15 years governance finance. MBA."
        )
        assert r.status == ProcessingStatus.COMPLETED
        assert r.attempts == 1
    ...
    def test_linked_to_document(self):
        assert (
            DocumentProcessor().process("doc-xyz", content="text").document_id
            == "doc-xyz"
        )
...
class TestProcessEndpoint:
    ...
    def test_nonexistent(self, client):
        assert (
            client.post("/api/v1/documents/bad/process", headers=_headers()).status_code
            == 404
        )
    ...
    def test_other_user(self, client):
        ...
        assert (
            client.post(
                f"/api/v1/documents/{doc_id}/process", headers=_headers("b")
            ).status_code
            == 404
        )
...
class TestProcessingStatusEndpoint:
    def test_before_processing(self, client):
        doc_id = _upload_pdf(client).json()["id"]
        resp = client.get(
            f"/api/v1/documents/{doc_id}/processing-status", headers=_headers()
        )
    ...
    def test_nonexistent(self, client):
        assert (
            client.get(
                "/api/v1/documents/bad/processing-status", headers=_headers()
            ).status_code
            == 404
        )
...
class TestExtractedFieldsEndpoint:
    def test_after_processing(self, client):
        ...
        DocumentProcessor(
            provider=TemplateExtractionProvider(), result_repo=_processing_repo
        ).process(doc_id, content="Jane Smith\n15 years governance finance. GAICD.")
        resp = client.get(
            f"/api/v1/documents/{doc_id}/extracted-fields", headers=_headers()
        )
    ...
    def test_nonexistent(self, client):
        assert (
            client.get(
                "/api/v1/documents/bad/extracted-fields", headers=_headers()
            ).status_code
            == 404
        )
...
class TestConfidenceAndReview:
    ...
    def test_low_confidence(self):
        assert ExtractedField(
            field_name="t", value="v", confidence=0.5, needs_review=True
        ).needs_review

    def test_no_auto_overwrite(self):
        for f in TemplateExtractionProvider().extract(
