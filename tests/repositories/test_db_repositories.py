...
SQLAlchemy-backed implementations, using an ephemeral in-memory SQLite
database created from the ORM metadata directly (not via Alembic, to keep
these tests fast and independent of a running database).

Limitation: this sandbox/CI environment does not have a real PostgreSQL
server available, and the ``psycopg`` binary wheel could not be installed
for this platform (see PR description). These tests therefore validate
correctness of the SQL generated by the DB-backed repositories using
SQLite as a stand-in dialect. They do **not** exercise Postgres-specific
behavior (e.g. the statement-timeout connection hook, which is skipped for
SQLite engines by design). Running the same suite against a real Postgres
instance is tracked as follow-up work.
"""
