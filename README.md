...
- **Azure AI Document Intelligence** — `AzureDocumentIntelligenceProvider` is the production hook point for CV/document extraction; the current provider raises a configuration-focused `NotImplementedError` until wired to a real service.
- **Microsoft Entra ID** — bearer-token authentication is available when `AUTH_ISSUER` and `AUTH_AUDIENCE` are configured.
- **Microsoft Graph** — integration and network modules model consent, token revocation, connection sync, and approved introduction-path usage.
